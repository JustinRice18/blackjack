...
function dealplayer()
{
    ...
    $randomkey = array_rand($_SESSION["deck"], $pick2 );
    if($_SESSION['player']=== 0)
    foreach ($randomkey as $key) {
        $end =implode($randomkey, " and ");


        echo "you received " .$end . " bringing your total to " . $_SESSION['deck'][$key], '<br />';
        ($_SESSION['player'] += $_SESSION['deck'][$key]);
    }
    elseif($_SESSION['player']>0 && $_SESSION['player'] < 21){
        $pick1 = rand(1,1);
        $randomkeys = array_rand($_SESSION["deck"], $pick1 );
        foreach ($randomkeys as $keys) {
            echo "you received " .$randomkeys. " bringing your total to " .( $_SESSION['player']+$keys); '<br />';
